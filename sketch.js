...
var backgroundImg
var score=0,highScore=0,count=0;
...
function setup() {
  ...
  invisibleGround = createSprite(0,height-10,width,125);  
  invisibleGround.shapeColor = "#f4cbaa";
  
  ground = createSprite(0,height,width,2);
...
function draw() {
  ...
  if (gameState===PLAY){
   ...
   if (ground.x < width/2){
    ...
    gameOver.x = camera.x; //P-38
    restart.x = camera.x; //P-38
  ...
  camera.x = trex.x; //P-38
  console.log(camera.x)
  drawSprites();
  
  textSize(20);
  fill("black")
  text("Score: "+ score,camera.x+250,50);
   // if(count===1)
    text("HS : "+ highScore, camera.x+150,50);
}
